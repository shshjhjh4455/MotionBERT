...
>>>>>>> 68806d2 (make output)
    ...
    if not opts.evaluate:
        if torch.cuda.is_available():
            ...
            optimizer = optim.AdamW(
                [
                    {
                        "params": filter(
                            lambda p: p.requires_grad, model.backbone.parameters()
                        ),
                        "lr": args.lr_backbone,
                    },
                    {
                        "params": filter(
                            lambda p: p.requires_grad, model.head.parameters()
                        ),
                        "lr": args.lr_head,
                    },
                ],
        ...
        for epoch in range(st, args.epochs):
            ...
            for idx, (batch_input, batch_gt) in tqdm(
